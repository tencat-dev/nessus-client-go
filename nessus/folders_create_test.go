...
func TestClient_FoldersCreate(t *testing.T) {
	tests := []struct {
		name    string
		options []Option
		req     *FoldersCreateRequest
		wantErr bool
	}{
		{
			name: "success",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.FoldersCreate(tt.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("FoldersCreate() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != nil && got.ID == 0 {
				t.Errorf("FoldersCreate() got = %v", got)
			}
		})
	}
}
