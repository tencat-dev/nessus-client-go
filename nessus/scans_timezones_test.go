...
func TestClient_ScansTimezones(t *testing.T) {
	tests := []struct {
		name    string
		options []Option
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.ScansTimezones()
			if (err != nil) != tt.wantErr {
				t.Errorf("ScansTimezones() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != nil && len(got.Timezones) == 0 {
				t.Errorf("ScansTimezones() got = %v", got)
			}
		})
	}
}
