...
func TestClient_FoldersGet(t *testing.T) {
	tests := []struct {
		name    string
		options []Option
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.FoldersGet()
			if (err != nil) != tt.wantErr {
				t.Errorf("FoldersGet() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != nil && len(got.Folders) == 0 {
				t.Errorf("FoldersGet() got = %v", got)
			}
		})
	}
}
